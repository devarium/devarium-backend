...
public class S3ImageStorage implements ImageStorage {
    ...
    public String upload(Image image, String key) {
        ...
        try (InputStream inputStream = new ByteArrayInputStream(image.getContent())) {
            log.debug("Uploading image to S3 - bucket: %s, key: %s, contentType: %s, size: %d"
                .formatted(bucket, key, image.getContentType(), image.getContent().length)
            );

            s3Client.putObject(new PutObjectRequest(bucket, key, inputStream, metadata));
            return s3Client.getUrl(bucket, key).toString();
        } catch (IOException e) {
            log.error("Failed to read image content - key: %s".formatted(key), e);
            ...
            log.error("S3 upload failed - bucket: %s, key: %s, errorCode: %s, errorMessage: %s"
                .formatted(bucket, key, e.getErrorCode(), e.getErrorMessage())
            );
    ...
    public void delete(String key) {
        try {
            log.debug("Deleting image from S3 - bucket: %s, key: %s".formatted(bucket, key));

            s3Client.deleteObject(bucket, key);
        } catch (AmazonS3Exception e) {
            log.error("S3 delete failed - bucket: %s, key: %s, errorCode: %s, errorMessage: %s"
                .formatted(bucket, key, e.getErrorCode(), e.getErrorMessage())
            );
